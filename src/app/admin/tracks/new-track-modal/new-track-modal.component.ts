...
export class NewTrackModalComponent implements OnInit {
    ...
    public upload_without_album: boolean = false;
    ...
    ngOnInit() {
        try {
            this.upload_without_album = this.data.track.withoutAlbum;
        } catch (e) {}
    }
     ...
     */
    public confirm() {
        let payload_data = this.getPayload();
        // validator
        if(this.upload_without_album === false && !payload_data.album_name) {
            let track_album: any = document.getElementById('track-album-name');
            track_album.focus();
            track_album.setCustomValidity("Album must be inputed!");
            return false;
        }
        ...
        //creating or updating track for new album
            ...
            if(payload_data.withoutAlbum === true) {
                this.create();
            }
            else {
     ...
     */
    public attachArtist(artistName: string) {
    ...
    public openUploadMusicModal() {
        ...
        openUploadWindow({types: [UploadInputTypes.audio, UploadInputTypes.video]}).then(uploadedFiles => {
            ...
            this.uploadQueue.start(uploadedFiles, params).subscribe(fileEntry => {
                ...
                this.track.name = fileEntry.name;
                this.artistsInput.formControl.patchValue(fileEntry.artist);
                this.attachArtist(fileEntry.artist);
    ...
    private getPayload() {
        ...
        payload.withoutAlbum = this.upload_without_album;
    ...
    private hydrate(params: CrupdateTrackModalData) {
        ...
        if(typeof (params.track) === 'object' && params.track.id > 0)
        {
            this.updating = true;
        }
        else
